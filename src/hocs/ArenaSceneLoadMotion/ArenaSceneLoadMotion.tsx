...
> {
    componentWillMount() {
        ...
        this.setState({
            ...
            styleCalculator: combineStyleCalculator(
                ...
                (phase: AnimationPhase) => setImmediate(() => this.props.actions.nextPhase(phase))
            ),
            playElement: this.props.bundle ? this.props.children(this.props.bundle) : null
        })
    }
    ...
    componentWillReceiveProps(
        ...
        if (this.props.bundle !== nextProps.bundle || this.props.children !== nextProps.children) {
            state.playElement = nextProps.bundle ? nextProps.children(nextProps.bundle) : null
        }
        ...
        if (nextProps.initStyles !== this.props.initStyles) {
            let nextPhaseStyle: any = state.initStyles.find(style => style.key === "nextPhase")
            if (nextPhaseStyle) {
    ...
    render() {
        ...
        return (
            <TransitionMotion defaultStyles={initStyles} styles={styleCalculator}>
                {interpolatedStyles => {
                    let containerStyle, scenePlayStyle, loadingPlayStyle, animationPhase: number
