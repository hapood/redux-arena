...
export function* applySceneBundle({
    ...
    let curtainState: CurtainState = yield select((state: any) => state[arenaCurtainReducerKey])
    let { curSceneBundle, reduxInfo, PlayingScene: OldPlayingScene, mutableObj } = curtainState
    ...
    let propsPicker = createPropsPicker(sceneBundle.propsPicker, newReduxInfo, newMutableObj)
    let PlayingScene = connect(propsPicker)(sceneBundle.Component)
    let displayName = sceneBundle.Component.displayName || sceneBundle.Component.name || "Unknown"
