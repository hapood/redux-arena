...
function* initArenaCurtainSaga({ reducerKey, setSagaTask }: InitArenaCurtainAction) {
...
function* killArenaCurtainSaga({ sagaTaskPromise, reducerKey }: KillArenaCurtainAction) {
    ...
    let { reduxInfo } = (yield select((state: any) => state[reducerKey])) as CurtainState
