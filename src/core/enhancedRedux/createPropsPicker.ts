...
function defaultPropsPicker(sceneState: any, sceneActions: ActionCreatorsMapObject) {
...
export default function createPropsPicker<P>(
    ...
    return (state: any) => {
        if (
            ...
            latestProps = propsPicker(state[sceneReducerKey], sceneActions, state, arenaReducerDict)
            return latestProps
        }
    }
}
