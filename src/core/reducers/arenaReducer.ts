...
function addStateTreeNode(state: ArenaState, pReducerKey: string, reducerKey: string) {
    ...
    if (pReducerKey == null) {
        ...
        newStateTreeDict = stateTreeDict.set(reducerKey, Map({ path: List([reducerKey]) }))
    } else {
        ...
        newStateTreeDict = stateTreeDict.set(reducerKey, Map({ path, isObsolete: false }))
    }
...
function deleteStateTreeNode(state: ArenaState, reducerKey: string) {
    ...
    if (node.get("children").count() === 0) {
        let path4Del = findNodeUpward(
            ...
            stateTreeDict,
            (pNode, dictItem) =>
                dictItem.get("isObsolete") === true && pNode.get("children").count() === 1
        )
        if (path4Del != null) {
...
export default function reducer(state = getArenaInitState(), action: AnyAction) {
    switch (action.type) {
